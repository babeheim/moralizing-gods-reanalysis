...
#SC variables: change "5:13" to "5:8" and then "9:13" in following: "ImpDat <- ImpDatRepl[ImpDatRepl$irep==irep,5:13]"
#Moralising god type: change "MoralisingGods" to "MoralisingHighGods" throughout in BigGodsAnalysesEditedV2.R
#Uncertainty: add following line between "MG<-..." and "MGAppear<-..." lines in BigGodAnalyses.R: "MG<-as.data.frame(MG %>% group_by(PolID) %>% sample_n(size = 1)) #randomly samples so there is only one century per polity" (add "library(dplyr)" before the loop, but note that R bugs when I load both dplyr and plyr)
...
#PC1[,5] <- -(PC1[,5]) #Used to correct arbitrarily flipped sign
#PC1[,20] <- -(PC1[,20]) #Used to correct arbitrarily flipped sign
...
################################################
# Calculate PC1_scale on ImpDat
#setwd("C:/Users/Peter Turchin/Google Drive/2.Seshat/1.R/PCA-MI")
nrep <- 20

ImpDat <- read.csv('ImpDatRepl.csv', header=TRUE)
PC1_scale <- matrix(NA,(length(ImpDat[,1])/nrep),0)
for(irep in 1:nrep){
  print(irep)
  ImpDat <- read.csv('ImpDatRepl.csv', header=TRUE)
  ImpDat <- ImpDat[ImpDat$irep==irep,5:8]
  res <- prcomp(ImpDat, scale=TRUE)
  print(summary(res))
  PCA<-predict(res)
  
  PC1_scale <- cbind(PC1_scale,PCA[,1])
}

ImpDat <- read.csv('ImpDatRepl.csv', header=TRUE)
out <- ImpDat[ImpDat$irep==irep,1:4]
out[,4] <- 4 + rowMeans(PC1_scale)
colnames(out) <- c("NGA","PolID","Time","PC1scale")
out$PC1scale <- (out$PC1scale - min(out$PC1scale))*10/(max(out$PC1scale)- min(out$PC1scale))

# Sort
dat <- out
out <- matrix(nrow = 0, ncol = 0)
NGAs <- c("Latium","Paris Basin","Iceland","Upper Egypt","Niger Inland Delta","Ghanaian Coast","Susiana","Konya Plain",
          "Yemeni Coastal Plain", "Kachi Plain","Deccan","Garo Hills","Sogdiana","Orkhon Valley","Lena River Valley",
          "Middle Yellow River Valley","Kansai","Southern China Hills", "Cambodian Basin","Central Java","Kapuasi Basin",
          "Big Island Hawaii","Chuuk Islands","Oro PNG","Valley of Oaxaca","Cahokia","Finger Lakes",
...
write.csv(out, file="PC1_scale.csv",  row.names=FALSE)
...
LogistRegrDat = as.data.frame(cbind(RD2$HS,RD2[,c(36,51:54)]))
source("LogistRegr.R")
