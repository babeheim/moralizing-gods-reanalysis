...
This document first replicates (section 1) and later extends (sections 2-4) the analyses published in the paper "Whitehouse, H., François, P., Savage, P. E., Currie, T. E., Feeney, K. C., Cioni, E., Purcell, R., Ross, R. M., Larson, J., Baines, J., ter Haar, B., Covey, A., Turchin, P. (2019). Complex societies precede moralizing gods throughout world history. Nature."
...
Note that comments in the text starting with #?? are our new comments. Also note that we are hailing from the original script, which uses a bit unfortunate approach that reuses and redefines the same variables. Hence, it may be misleading to run a specific section multiple times (it is going to overwrite results) or start from the middle of the script. As a general rule, we recommend to always run the full code that precedes the section that the reader might be interested in.
...
```{r cache=F, results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.height=5, fig.width=5}
...
```{r cache=F, results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.height=20, fig.width=6}
...
```{r cache=F, results='hide', message=FALSE, warning=FALSE, echo=FALSE}
...
ggplot() + 
 ...
 #ggsave("hist100.tiff",width = 4,height = 2.7, dpi = 300)
...
#tiff("qq1.tiff",width = 8,height = 5, units = 'in', res = 300)
...
#####################################################################
## Function extracting coefficients and calculating 95% CI from various models

xtract.cof <- function(model,number,extension,package,transform = 0){
  ...
  # used. Currently available for "glm","glmer", "glmmadmb", and "glmmTMB"
  
  # Transform specifies whether coefficients and 95% CIshould be
  # back-transformed from log/logit links
  # 0 = no transformation, output coefs, CI, and p-vals;
  # 1 = exponentiate coefs and CI (for logit link in logistic regrssion
  # calculates Odds ratios)
  # 2 = back-transforms coefs and CI from the log link in negative binomial
  # regression
  # 3 = back-transforms coefs and CI from the logit link in beta regression
  # (currently available only for "glmmadmb" package)
    ...
    for(i in number){
      ...
      if(transform == 1){
        ci <- exp(cbind(OR = cf, LL = cf - 1.96 * se,
                        UL = cf +  1.96 * se))
      }else if(transform == 0) {ci <- (cbind(est = cf, LL = cf - 1.96 * se,
                                      UL = cf +  1.96 * se))
      }else if(transform == 2){ci <- (cbind(est = cf, LL = se,
                                     UL = se))
      }            
...
Let's look also at the appearance of writing and whether it will correspond to the same sudden jump in SC.
...
dat.W$Time[dat.W$NGA == "Deccan" & (dat.W$Time == 3200)] <- 3100
dat.W$Time[dat.W$NGA == "Deccan" & (dat.W$Time == 3300)] <- 3200
...
for(i in 1:length(NGAs)){
  dt <- dat.W[dat.W$NGA == NGAs[i],]
  m <- max(dt$Time, na.rm=T)
  dat.W$Time[dat.W$NGA == NGAs[i]] <- dat.W$Time[dat.W$NGA == NGAs[i]] - m
...
ggplot(dat.W, aes(Time, Mean, color=Writing)) + 
  ...
  scale_color_manual(values = alpha(c("darkviolet","deepskyblue"), .8), labels = c("Missing", "Present"), name = "Writing") + 
  scale_x_continuous(breaks = c(0,200,400,600,700,800,1000,1200,1400),
                     labels = c("-700","-500","-300","-100","0","100","300","500","700")) +
  geom_vline(xintercept = 650,
                color = "grey", size=10, alpha = 0.5) + 
...
#ggsave("PrePost_writing.tiff",width = 6,height = 5.5, dpi = 300)
...
```{r results = 'hide', message=FALSE, warning=FALSE, cache=F} 
...
xtract.cof("glm.b",c(1:3),"","glmmadmb",3)
...
#tiff("qq2.tiff",width = 7,height = 5, units = 'in', res = 300)
plot(simulationOutput)
#dev.off()
testUniformity(simulationOutput = simulationOutput)
testDispersion(simulationOutput = simulationOutput)
...
simulationOutput = simulateResiduals(glm.b3)
#tiff("qq3.tiff",width = 8,height = 5, units = 'in', res = 300)
plot(simulationOutput)
#dev.off()
testUniformity(simulationOutput = simulationOutput)
testDispersion(simulationOutput = simulationOutput)
