---
title: "Reanalysis of 'Complex societies precede moralizing gods' causal analysis"
output:
  html_document:
    code_folding: show
    theme: spacelab
...
This document first replicates and later extends the analyses published in the paper "Whitehouse, H., François, P., Savage, P. E., Currie, T. E., Feeney, K. C., Cioni, E., Purcell, R., Ross, R. M., Larson, J., Baines, J., ter Haar, B., Covey, A., Turchin, P. (2019). Complex societies precede moralizing gods throughout world history. Nature."
...
{
##? Load libraries
libs <- c("dplyr", "glmmADMB", "glmmTMB", "nlme", "lme4", "plotrix", "ggplot2", "stargazer", "DHARMa", "bbmle", "knitr", "car", "bootpredictlme4", "reshape", "grid", "yarrr")
...
ggplot(data = coefs, aes(x=prepost, y=mean.DV))   +
  geom_jitter(data = out.s, alpha=0.3, aes(x=prepost, y=Rate, colour = TimeWindow), width =
...
ggplot() + 
   geom_density(data = out.s, aes(x = Rate, fill = prepost, colour = prepost, linetype=prepost),size = 0.5) +
...
In this section, we will keep up with the inappropriate t-test, but try to correct for forward bias, i.e., the fact that if we take the first known occurence of MG, it is very unlikely that it is actually the oldest occurence. Let's try to shift the first occurence of each MG one and three centuries back.
...
##?
  for(j in 1: 100){
    ...
    rates<-cbind(MGAppear$NGA,ifelse(class(Earliest$Time)=="NULL","NA",
                 ...
                 ifelse(class(Latest$Time)=="NULL","NA",((Latest$Mean-MGAppear.FB$Mean)/(Latest$Time-MGAppear.FB$Time))),
                 (MGAppear$End-MGAppear$Start),j*100,MGAppear$World.Region,MGAppear$Family)
    out <- rbind(out,rates)
  }
  out <- rbind(out,rates)
...
ggplot() + 
  geom_histogram(data = d, aes(x = d), fill = "darkgrey", colour = "gray42", binwidth = 0.1, alpha = 0.9) +
#  geom_histogram(data = d100, aes(x = d100), fill = "#cf3a3a", colour = "#cf3a3a", binwidth = 0.1, alpha = 0.5) +
#  geom_histogram(data = d300, aes(x = d300), fill = "#18b0e8", colour = "#18b0e8", binwidth = 0.1, alpha = 0.4) +
  
 
# scale_color_manual(values = alpha(c("gray42","#cf3a3a","#18b0e8"),1)) +
#   scale_fill_manual(values = alpha(c("darkgrey","#cf3a3a","#18b0e8"),0.5),
#                     labels = c("MG 0", "MG -100", "MG -300"), name = "") +
                   xlim(c(-2,2)) +
  ylim(c(0,100)) +
...
FB <- 100 ##? again, this is our forward bias correction
...
for(i in 1:length(NGAs)){
  ...
  MG<-subset(dt,MoralisingGods=="1") #Replace "MoralisingGods" with "DoctrinalMode" or "Writing" to do these analyses
  MGAppear<-subset(MG, Time==min(Time))
  dt$Time.norm<-dt$Time-(MGAppear$Time-FB)
  out <- rbind(out,dt)
}
...
## Function extracting coefficients from various models
 ...
 xtract.cof <- function(model,number,extension,package,OR = 1){
    ...
    # used. Currently available for "glm","glmer",and "glmmadmb"
...
ggplot() + 
  geom_density(data = dat.p, aes(x = Mean, fill = MoralisingGods, colour = MoralisingGods, linetype=MoralisingGods),size = 0.5) +
...
# Define list of coefficients extracted from glmmadmb models: estimates, confidence intervals, and p-values
